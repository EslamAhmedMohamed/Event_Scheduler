...
#include<ctime>
using namespace std;
//i had modified  fun (show done events) to cout "there is no done events" if so not an empty table 
void Event::start() {
...
{//this fun retern event to be add at insert fun
	Event TempEvent;	
...
void Event::Insert(Event e)
{
	...
	if (Events.size() == 0)
	{
		...
		for (i = 0; it < Events.end(); i++, it++)
		{
			...
			{
				if (Events.back().end.Month > e.Start.Month || (Events.back().end.Month == e.Start.Month && Events.back().end.Month >= e.Start.Month))
				{
					cout << "Sorry This Time Alrady Contain An Event" << endl;
					break;
				}
			...
			{
				cout << "u suppose to be in this event";
				break;
			}
		}
	}
...
void Event::done_events() {
	for (int i = 0; i < Events.size(); i++) {
		if (Events[i].CalcReminderInDays(Events[i].Reminder(Events[i].Start, Events[i].Current)) == 0 &&(Events[i].end.Month> Events[i].Current.Month|| Events[i].end.Day> Events[i].Current.Day)) {
			Events[i].Done = true;
...
/* 1 - make fun addfromuser add event to file and it haven't return type
   2 - make update and delete fun do that in files 
   3 - don't insert events to arrayList,only when call showEvents fun
*/